...
import type { BriefRoles, Database, Json } from "@/lib/database.types";
...
export default function ReviewPage() {
  ...
  const loadQueue = useCallback(async () => {
    ...
    const { data, error: queueError } =
      await backend.rpc<QueueRow[]>("review_queue");
  ...
  const loadOversight = useCallback(async () => {
    ...
    setHasOversight(Boolean(data?.has_oversight));
  }, []);
  ...
  useEffect(() => {
    ...
    const loadForUser = async (nextUser: User | null) => {
      ...
      await Promise.all([loadQueue(), loadOversight()]);
    };
  ...
  useEffect(() => {
    if (!assessmentId || authState !== "authenticated") {
      setReviewStatuses([]);
      return;
    }
    let active = true;
    void backend
      .rpc<ReviewStatusRow[]>("assessment_review_status", {
  ...
  // The marking site stores the URL it turned away and returns here after
  // login, query string and all, so there is nothing to hand it.
  const signIn = async () => {
    setError(null);
  ...
  const recordDecision = async (decision: "approve" | "withdraw") => {
    if (!user || !selectedRow || !selectedRow.can_review) return;
    ...
    setError(null);
    setNotice(null);
    const { error: reviewError } = await backend.rpc(
  ...
  if (authState === "signed-out" || !user) {
    return (
      <StatePage
        ...
        body="Sign in to the marking site to view this assessment."
      >
